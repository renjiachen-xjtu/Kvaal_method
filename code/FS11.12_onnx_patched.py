...
# === 配置区：请确保路径正确 ==
EXCEL_PATH = r"D:\Users\HUAWEI\Desktop\Age_Estimation_Project\male.xlsx"
OUTPUT_DIR = r"D:\Users\HUAWEI\Desktop\Age_Estimation_Project\models\male"  # 新增图表输出目录
...
def main():
    try:
        ...
        for sheet in sheet_names:
            ...
            # ======== 导出 ONNX（每个牙位/工作表一个文件）========
            MODELS_DIR = os.path.join(OUTPUT_DIR, "models", "female")  # 若你跑 male.xlsx，把 female 改成 male
            os.makedirs(MODELS_DIR, exist_ok=True)

            safe_sheet_name = safe_filename(sheet)
            onnx_path = os.path.join(MODELS_DIR, f"rf_{safe_sheet_name}.onnx")

            initial_type = [("input", FloatTensorType([None, X_train.shape[1]]))]  # 输入维度=特征数（通常10）
            onnx_model = convert_sklearn(best_model, initial_types=initial_type, target_opset=17)

            with open(onnx_path, "wb") as f:
                f.write(onnx_model.SerializeToString())

            # 同时保存特征顺序（网页端必须按这个顺序喂数据）
            feat_path = os.path.join(MODELS_DIR, f"rf_{safe_sheet_name}.features.json")
            with open(feat_path, "w", encoding="utf-8") as f:
                json.dump(list(X.columns), f, ensure_ascii=False, indent=2)

            log_and_print(f"  ✅ RF ONNX已导出: {onnx_path}")
